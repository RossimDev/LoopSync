...
async function generateSyncVideo({
  ...
  try {
    ...
    if (copyVideo) {
      const { args: retryArgs } = buildArgs({
        videoPath,
        audioPath,
        outputPath,
        audioDuration: duration,
      ...
      await runFFmpeg(retryArgs, { onProgress: handleProgress });
    } else {
      throw err;
    }
  }
  ...
  const actualDuration = infoOutput.duration;
  const diffMs = Math.round(Math.abs((actualDuration - duration) * 1000));
